...
additional_stop_words = ['advertisement', 'advertisements', 'cup', 'cups',
...
def create_embeddings(dataset):
    ...
    for cat in np.unique(labels):
        ...
        dtf_features = dtf_features.append(pd.DataFrame({'feature': names, 'score': 1-p, 'labels': cat}))
    ...
    for cat in np.unique(labels):
        print('# {}:'.format(cat))
        print('  . selected_features:', len(dtf_features[dtf_features['labels'] == cat]))
