...
def welcome(message):
    ...
    item4 = types.KeyboardButton('Computer')
...
def dialog(message):
    if message.chat.type == 'private':
        if message.text == 'Random Number':
            ...
            from beautiful_soup import scrape_data
            phones = scrape_data()
            bot.send_message(message.chat.id, random.choice(phones))
        elif message.text == 'Computer':
            from beautiful_soup import computer
            computer = computer()
            bot.send_message(message.chat.id, random.choice(computer))
...
def callback_inline(call):
    try:
        if call.message:
            ...
            # show alert
            bot.answer_callback_query(callback_query_id=call.id, show_alert=True, text='Text Alert')
